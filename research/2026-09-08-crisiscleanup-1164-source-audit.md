...
| External AI contributions are permitted under known terms. | D, unresolved: AI-tool instructions exist, but no explicit external policy was established. | Confirm disclosure expectations and retrieve actual CLA text before implementation. |
...
shell to the read-only web fetch, so actual CLA terms remain unverified. Current
